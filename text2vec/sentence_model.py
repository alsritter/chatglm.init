...
class SentenceModel:
    def __init__(
        ...
        logging.debug("Use device: {}".format(self.device))
    ...
    def get_sentence_embeddings(self, input_ids, attention_mask, token_type_ids=None):
        ...
        if self.encoder_type == EncoderType.FIRST_LAST_AVG:
            ...
            first_avg = torch.avg_pool1d(first.transpose(
                1, 2), kernel_size=seq_length).squeeze(-1)  # [batch, hid_size]
            last_avg = torch.avg_pool1d(last.transpose(
                1, 2), kernel_size=seq_length).squeeze(-1)  # [batch, hid_size]
            final_encoding = torch.avg_pool1d(
                torch.cat([first_avg.unsqueeze(1), last_avg.unsqueeze(
                    1)], dim=1).transpose(1, 2),
        ...
        if self.encoder_type == EncoderType.LAST_AVG:
            ...
            final_encoding = torch.avg_pool1d(sequence_output.transpose(
                1, 2), kernel_size=seq_length).squeeze(-1)
            return final_encoding
    ...
    def evaluate(self, eval_dataset, output_dir: str = None, batch_size: int = 16):
        ...
        for batch in tqdm(eval_dataloader, disable=False, desc="Running Evaluation"):
            ...
            source_input_ids = source.get(
                'input_ids').squeeze(1).to(self.device)
            source_attention_mask = source.get(
                'attention_mask').squeeze(1).to(self.device)
            source_token_type_ids = source.get('token_type_ids', None)
            if source_token_type_ids is not None:
                source_token_type_ids = source_token_type_ids.squeeze(
                    1).to(self.device)

            # target        [batch, 1, seq_len] -> [batch, seq_len]
            target_input_ids = target.get(
                'input_ids').squeeze(1).to(self.device)
            target_attention_mask = target.get(
                'attention_mask').squeeze(1).to(self.device)
            target_token_type_ids = target.get('token_type_ids', None)
            if target_token_type_ids is not None:
                target_token_type_ids = target_token_type_ids.squeeze(
                    1).to(self.device)
        ...
        logging.debug(f"labels: {batch_labels[:10]}")
        logging.debug(f"preds:  {batch_preds[:10]}")
        logging.debug(f"pearson: {pearson}, spearman: {spearman}")
    ...
    def save_model(self, output_dir, model, results=None):
        ...
        :return:
        """
        logging.debug(f"Saving model checkpoint to {output_dir}")
        os.makedirs(output_dir, exist_ok=True)
    ...
    def encode_multi_process(
        ...
        for sentence in sentences:
            ...
            if len(chunk) >= chunk_size:
                input_queue.put([last_chunk_id, batch_size,
                                chunk, normalize_embeddings])
                last_chunk_id += 1
                chunk = []
